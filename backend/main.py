...
from .services.folder_compare import match_file_pairs, build_folder_tree
from .services.diff_service import (
    compare_files,
    generate_diff_summary,
    compare_files_metadata,
)
from .services.excel_service import (
    update_excel_file,
    write_changes_to_excel,
    add_diff_image_to_excel,
    sheet_name_from_component,
    write_delta_groups_to_excel,
)
...
async def compare_files_endpoint(
    request: Request,
    old_path: str = Form(None),
    new_path: str = Form(None),
    ...
    if old_file is not None and new_file is not None:
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Error reading uploaded files: {exc}"
            )
        ...
        unified_diff = list(
            difflib.unified_diff(
                old_lines,
                new_lines,
                fromfile=old_file.filename or "old",
                tofile=new_file.filename or "new",
                lineterm="",
            )
        )
        ...
        summary = (
            ...
            if not has_changes
                ...
                [
                    p
                    for p in (
                        f"{added} line(s) added" if added else None,
                        f"{removed} line(s) removed" if removed else None,
                    )
                    if p
                ]
            )
    ...
    if "application/json" in content_type:
        try:
            ...
            data = None
    if data is None and old_path and new_path:
        data = {"old_path": old_path, "new_path": new_path}

    if not data or not data.get("old_path") or not data.get("new_path"):
        raise HTTPException(
            status_code=400,
            detail="Provide either two uploaded files or JSON/form with old_path and new_path",
        )
...
async def scan_folders_endpoint(request: Request):
    ...
    if not old_folder or not new_folder:
        raise HTTPException(
            status_code=400, detail="old_folder and new_folder are required"
        )
    ...
    matched_pairs, old_only_files, new_only_files = match_file_pairs(
...
async def compare_folders_endpoint(request: CompareFoldersRequest):
    ...
    def walk_and_summarize(node, current_rel_path: str = ""):
        ...
        for f in node.get("files", []):
            ...
            file_summary["semantic_diff"] = {
                "changes": [],
                "summary": md.get("semantic_summary", {}),
            }
            file_summaries.append(file_summary)

        for sub in node.get("subfolders", []):
            walk_and_summarize(
                sub, os.path.join(current_rel_path, sub.get("name", ""))
            )
    ...
    new_tree = build_folder_tree(new_root)
    ...
    unique_components = set(
        [
            os.path.dirname(os.path.relpath(fs["old_path"], old_root))
            or os.path.basename(old_root)
            for fs in file_summaries
        ]
    )
...
async def compare_files_upload(
    ...
    added = sum(
        1 for line in unified_diff if line.startswith("+") and not line.startswith("+++")
    )
    removed = sum(
        1 for line in unified_diff if line.startswith("-") and not line.startswith("---")
    )
...
async def save_edited_file_endpoint(payload: dict):
    ...
    if not file_path or updated_content is None:
        raise HTTPException(
            status_code=400, detail="file_path and updated_content are required"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Syntax error detected for {ext} file: {exc}",
        )
    ...
    if dirpath and not os.path.exists(dirpath):
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Unable to create directory for file: {exc}",
            )
...
async def write_changes_endpoint(request: WriteChangesRequest):
    ...
    success, message, written_rows = write_changes_to_excel(
        request.excel_path, request.changes
    )
    return WriteChangesResponse(
        success=success, message=message, written_rows=written_rows
    )
...
async def write_diff_image_endpoint(
    ...
    if not image_bytes:
        raise HTTPException(
            status_code=400, detail="Invalid image upload: empty file"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid image upload: {exc}"
        )
...
def browse_folder_dialog():
    ...
    try:
        ...
        return {
            "success": False,
            "message": "Error opening folder selection dialog",
        }
